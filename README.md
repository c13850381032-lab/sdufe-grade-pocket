...
1. 教务系统“成绩信息”横向表：保留原始前三行，课程表头中包含课程名称、学分和学期。
2. “课程 + 学分”成对表：前三列依次为学号、姓名、学期，后面每门课程成绩紧跟一列学分。
3. 通用明细表：包含学号或姓名、课程名称、学分、成绩，建议同时包含学期。

必修课只以当前用户本人查询结果中的“必修”标注为依据。系统按课程名称和学期匹配班级成绩；班级 CSV 自带的课程性质和选课人数均不参与必修认定。因此，请先刷新本人成绩，再导入班级 CSV。导入的数据和计算结果仅保存在当前电脑的浏览器中，不会上传到 GitHub 或其他服务器。
